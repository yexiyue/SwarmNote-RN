...
- [构建管线：从 TypeScript 到 WebView 加载](#构建管线从-typescript-到-webview-加载)
- [实战代码走读](#实战代码走读)
...
graph TB
    ...
    subgraph "WebView 进程"
        E["editor-web 单文件 HTML<br/>（vite + singlefile 打包）"]
        F["createWebViewEndpoint()<br/>（WebView 侧 Endpoint 适配器）"]
        G["createEditorRuntime(host)<br/>（暴露 EditorApi 服务）"]
...
| **Bundle 层** | 编辑器代码打包为单 HTML | `editor-web/`（`comlink-endpoint.ts` / `editor-runtime.ts` / `index.ts`） |
...
## 构建管线：从 TypeScript 到 WebView 加载

WebView 是一个独立的浏览器环境，它不能访问 RN 的 `node_modules`。所以我们需要把编辑器代码和所有依赖打包成一个**自包含的 HTML 文件**，然后加载它。

### 构建流水线

```mermaid
graph LR
    subgraph "Step 1: Vite 打包"
        S1["src/index.ts<br/>+ 全部依赖<br/>（Comlink, CM6, Yjs…）"]
        S1 --> S2["vite-plugin-singlefile<br/>内联所有 JS/CSS<br/>到 index.html"]
    end
    
    subgraph "Step 2: Metro require"
        S2 --> S3["dist/index.html<br/>单文件 HTML<br/>约 640KB"]
        S3 --> S4["MarkdownEditor.tsx<br/>require() 拿到 HTML asset<br/>WebView source={{ uri }}"]
    end
...
`vite-plugin-singlefile` 把所有 JS 和 CSS 内联到 `index.html` 中，产出一个完全自包含的 HTML 文件。现代 Android/iOS WebView 支持 `<script type="module">`，无需转换为 IIFE 格式。
...
// 通过 expo-asset 下载到本地文件系统
const asset = Asset.fromModule(EDITOR_HTML_MODULE);
await asset.downloadAsync();

// WebView 直接加载本地 HTML 文件
<WebView source={{ uri: asset.localUri }} ... />
```
...
sequenceDiagram
    ...
    Note over Metro: require("editor-web/dist/index.html")<br/>→ 注册为 asset 资源
    RN->>RN: expo-asset 下载 HTML<br/>到本地文件系统
    RN->>WV: 创建 WebView<br/>source={{ uri: localUri }}
    Note over WV: 加载 HTML<br/>执行内联 &lt;script type="module"&gt;<br/>Comlink 连线 + expose(editorApi)
    WV->>RN: host.onRuntimeReady()<br/>（通过 Comlink RPC 通知就绪）
    RN->>WV: editorApi.createEditor(options)<br/>（通过 Comlink RPC）
    Note over WV: 创建 EditorView<br/>挂载到 #editor-root
```

相比早期 Joplin 采用的 IIFE → 字符串 → `injectedJavaScript` 注入模式，HTML 文件直接加载更简洁，且支持 ES module、source map 等现代特性。
...
`packages/editor-web/src/` 按职责拆为三个模块：

- `comlink-endpoint.ts` — Endpoint 适配器（与 RN 侧 `comlink-webview-adapter.ts` 对称）
- `editor-runtime.ts` — 编辑器运行时，实现 `EditorApi`
- `index.ts` — 入口，负责 Comlink 连线和环境检测

```mermaid
graph TB
    subgraph "WebView 启动流程"
        A["HTML 加载"] --> B["index.ts 执行"]
        B --> C["registerTransferHandlers()"]
        C --> D["createWebViewEndpoint() × 2"]
        D --> E["Comlink.wrap&lt;HostApi&gt;()"]
        E --> F["createEditorRuntime(host)"]
        F --> G["Comlink.expose(runtimeApi)"]
        G --> H["host.onRuntimeReady()"]
        H --> I["等待 RN 调用 createEditor()..."]
    end
```
...
// editor-runtime.ts — 编辑器运行时（工厂函数）
export function createEditorRuntime(host: HostApi): EditorApi {
  const state = { editor: null, ydoc: null, ... };

  return {
    createEditor(options) {
      state.editor = createEditor(root, { ...options, onEvent: host.onEditorEvent });
    },
    getText() { return state.editor.getText(); },
    setText(text) { state.editor.setText(text); },
    // ...更多方法
  };
}
```
...
`src/components/editor/useEditorBridge.ts` 管理整个桥接生命周期。通过两个通道实现双向 RPC：`editor-runtime`（RN → WebView）和 `editor-host`（WebView → RN）。

```mermaid
graph TB
    subgraph "useEditorBridge 初始化"
        A["useMemo 创建双通道 endpoint"] --> B["createRNEndpoint('editor-runtime')"]
        A --> B2["createRNEndpoint('editor-host')"]
        B --> C["Comlink.wrap&lt;EditorApi&gt;()"]
        B2 --> D["Comlink.expose(hostApi)"]
    end
    
    subgraph "消息接收"
        E["WebView.onMessage 触发"] --> F["JSON.parse(event.data)"]
        F --> G["两个 endpoint 各自<br/>dispatchMessage(data)"]
        G --> H["根据 channel 字段<br/>分流到对应 Comlink 处理"]
    end
```
...
export function useEditorBridge(options) {
  ...
  useEffect(() => {
    const hostApi: HostApi = {
      ...
      onEditorEvent(event) { options.onEditorEvent?.(event); },
    };
    Comlink.expose(hostApi, hostEndpoint);
  }, [hostEndpoint]);

  // 消息分发给两个通道，由 envelope 的 channel 字段自动路由
  const onWebViewMessage = useCallback((event) => {
    runtimeEndpoint.dispatchMessage(event.nativeEvent.data);
    hostEndpoint.dispatchMessage(event.nativeEvent.data);
  }, []);
...
export function MarkdownEditor({ initialText, enableYjs, ... }) {
  ...
  // Runtime 就绪 → 创建编辑器
  useEffect(() => {
    if (!editorApi || !runtimeReady) return;
    editorApi.createEditor(options);
  }, [runtimeReady]);

  return (
    <WebView
      source={{ uri: htmlUri }}        // 加载本地 HTML 文件
...
### 2. WebView 只能传字符串 + Uint8Array 需要 transferHandler

React Native WebView 的 `postMessage` 只接受字符串，不支持结构化克隆。所以 Endpoint 适配器里需要手动 `JSON.stringify` / `JSON.parse`。

`Uint8Array` 经过 `JSON.stringify` 会丢失类型，变成 `{"0":1,"1":2,...}` 的普通对象。我们通过注册 Comlink 自定义 `transferHandler` 解决：

```typescript
Comlink.transferHandlers.set('uint8array', {
  canHandle: (val) => val instanceof Uint8Array,
  serialize: (val) => [Array.from(val), []],   // Uint8Array → number[]
  deserialize: (val) => new Uint8Array(val),    // number[] → Uint8Array
});
```

**注意**：RN 和 WebView 两端都需要注册同样的 transferHandler，否则一端序列化了另一端不认识。

### 3. injectJavaScript 的安全传值

RN → WebView 方向通过 `injectJavaScript()` 注入代码。直接把 `JSON.stringify` 结果插入模板字面量会被反引号 `` ` `` 等字符打断，导致语法错误。正确做法是双重 stringify：

```typescript
const escaped = JSON.stringify(JSON.stringify(envelope)); // 安全的 JS 字符串字面量
webview.injectJavaScript(
  `window.dispatchEvent(new MessageEvent('message',{data:JSON.parse(${escaped})}));true;`
);
```
...
WebView 发回的消息可能属于不同的 Comlink 通道（`editor-runtime` 或 `editor-host`），也可能是非 Comlink 的调试日志。通过 envelope 的 `channel` 字段自动路由：

```typescript
// onWebViewMessage 把消息同时交给两个 endpoint
// 每个 endpoint 只处理自己 channel 的消息，忽略其他
runtimeEndpoint.dispatchMessage(raw);
hostEndpoint.dispatchMessage(raw);
```

`__debugLog` 等非 Comlink 消息在分发前拦截处理。
...
graph TB
    subgraph "我们做了什么"
        ...
        C["Vite 单文件 HTML 打包<br/>WebView 直接加载"] --> D["完整的 CM6 编辑器<br/>在 RN WebView 中运行"]
        E["Yjs 集成到 WebView 内<br/>通过 Comlink + transferHandler<br/>传递 Uint8Array update"] --> F["P2P 实时协作<br/>多设备同步编辑"]
    end
